...
float blurredPrevInk(vec2 uv) {
    vec2 texel = 1.0 / max(uPrevResolution, vec2(1.0));
    vec2 dx = vec2(texel.x * 3.5, 0.0);
    vec2 dy = vec2(0.0, texel.y * 3.5);

    vec3 color = texture2D(uPrevTex, uv).rgb * 0.28;
    color += texture2D(uPrevTex, uv + dx).rgb * 0.18;
    color += texture2D(uPrevTex, uv - dx).rgb * 0.18;
    color += texture2D(uPrevTex, uv + dy).rgb * 0.18;
    color += texture2D(uPrevTex, uv - dy).rgb * 0.18;

    return clamp(1.0 - luminance(color), 0.0, 1.0);
}

float blurredNextInk(vec2 uv) {
    vec2 texel = 1.0 / max(uNextResolution, vec2(1.0));
    vec2 dx = vec2(texel.x * 3.5, 0.0);
    vec2 dy = vec2(0.0, texel.y * 3.5);

    vec3 color = texture2D(uNextTex, uv).rgb * 0.28;
    color += texture2D(uNextTex, uv + dx).rgb * 0.18;
    color += texture2D(uNextTex, uv - dx).rgb * 0.18;
    color += texture2D(uNextTex, uv + dy).rgb * 0.18;
    color += texture2D(uNextTex, uv - dy).rgb * 0.18;
...
void main() {
    ...
    vec2 reverseUv = vec2(1.0 - uv.x, uv.y);
    float prevInk = blurredPrevInk(reverseUv);
    ...
    float translucency = mix(0.017, 0.045, cloudy);
    result -= neighborInk * exposedPaper * translucency * s * vec3(0.92, 0.86, 0.72);
